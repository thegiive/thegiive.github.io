...
categories: [AI 產業分析]
tags: [Anthropic, Claude, watermark, C2PA, EU AI Act, 透明度, AI 治理]
author: Wisely Chen
---
...
第二行混入了西里爾字母。`Clаude` 裡的 `а` 是 `U+0430`，`саn` 的前兩個字元分別是 `U+0441` 和 `U+0430`。兩個字串因此不相等：`"Claude" === "Clаude"` 結果是 `false`。
...
2026 年 7 月，有開發者[逆向工程 Claude Code v2.1.197 的 `claude.exe`](https://www.toutiao.com/article/7497975529413730851/)，在偏移量 `0xd2e2879` 附近找到一段打包後的 JavaScript。這段程式碼做了幾件事：

1. 讀取環境變數 `ANTHROPIC_BASE_URL`，解析出 hostname
2. 判斷本機時區是不是 `Asia/Shanghai` 或 `Asia/Urumqi`（注意：不是 UTC+8，香港、台灣、新加坡不會命中）
...
這 11 個 AI 關鍵詞解密後是：`deepseek`、`moonshot`、`minimax`、`xaminim`、`zhipu`、`bigmodel`、`baichuan`、`stepfun`、`01ai`、`dashscope`、`volces`——基本對應中國頭部 AI 模型廠商或平台。147 項域名名單的第一項就是 `cn`，加上匹配邏輯 `e===r || e.endsWith("."+r)`，所有 `.cn` 結尾的 hostname 都會命中。名單裡還有 `baidu.com`、`alibaba-inc.com`、`bytedance.net`、`bilibili.co` 等大公司域名，以及大量 API 中轉站域名。
