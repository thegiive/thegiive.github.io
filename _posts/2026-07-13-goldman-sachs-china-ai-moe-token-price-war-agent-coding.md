...
但激活比例低只是成本方程式的一部分。4-8 倍的價差不能全歸因於架構——**定價策略和利潤率的差異可能更大。** 美國 SOTA 模型定價 $4-8/百萬 token，裡面包含了高毛利（OpenAI 的 API 毛利率被估計在 50% 以上）；中國模型定價 $1 甚至更低，GS 測算是虧損狀態。同樣都用 MoE，一邊在賺錢，一邊在貼錢搶市場——**價差的本質是利潤率差異加上刻意的低價策略，不只是架構優勢。**
...
1. **4-8 倍的價差是「架構激進度 + 虧損定價」的疊加，不是單一因素。** 大家都用 MoE，但中國模型把激活比例壓到 3-5%，再疊上 -30% 到 -39% 的虧損定價。這兩層加起來才有 4-8 倍的價差。如果你在做成本規劃，要分開看：架構帶來的成本優勢大概是 2-3 倍，剩下的是補貼——補貼會結束，架構優勢不會。
