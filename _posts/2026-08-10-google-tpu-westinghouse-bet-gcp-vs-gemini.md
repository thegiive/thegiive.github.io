...
看 2026 年的前沿模型排名。Claude Opus 4.8、GPT-5.5、Gemini 3.1 Pro、Grok 4.3——LMArena 排行榜上，整個頂端只差 55 Elo。不同 benchmark 各有領先：Claude 贏 coding，Gemini 贏 abstract reasoning（ARC-AGI-2）和科學推理（GPQA 94.3%），Grok 贏 HLE。

換句話說，前沿模型正在趨向同質化。差距是歷史上最小的。

如果前沿模型趨向同質化，那模型本身就不是長期護城河——基礎設施才是。誰擁有晶片、誰有產能、誰的單位成本最低，才決定誰能大規模部署。

Westinghouse 的歷史支持這個論點。Edison 發明了燈泡，但電力產業的贏家是把交流電基礎設施鋪到全美國的 Westinghouse。發明者不一定贏，散佈者才贏。

如果這個類比成立，Google 正在做的事情就不是「放棄模型」，而是「看穿模型會變成 commodity，搶先佔位基礎設施」。
