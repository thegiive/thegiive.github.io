...
**日常任務層：性價比導向的中型模型。** Qwen 3 27B/35B 是目前最佳選擇之一，推理能力強、部署成本低，單張消費級 GPU 就能跑。美系的話 Gemma 4 也是同級別的選項。如果你需要更大的火力，Ornith-1.0-397B-FP8 這類社群微調模型也值得關注——397B 參數量用 FP8 壓到可部署的範圍，在開源社群裡正在快速迭代。這一層處理你 80% 的日常 Agent 任務：文件處理、code review、客服對話、資料分析。

**高推理層：開源大模型。** 當任務需要深度推理——複雜的多步驟規劃、跨文件的架構設計、長鏈條的邏輯推演——就上 GLM-5.2（1M context、MIT 開源）或 DeepSeek V4 這個級別的模型。這些模型的推理能力已經逼近閉源前沿，而且完全在你的掌控之下。
...
這套架構的好處很明確：**每一層都是開源的、每一層都在你的機房裡、沒有任何一層需要政府的許可。** 而且因為是分層設計，單一模型被替換或升級的時候，不會影響整個系統。
