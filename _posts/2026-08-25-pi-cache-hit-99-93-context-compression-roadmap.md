...
內建的 compaction 是最被動的一種：等 context 滿了才動。放眼整個開源生態，大家對「AI 到底該怎麼記憶」根本沒有共識——這是目前社群爭論最激烈的技術分歧。我整理出五種完全不同路線的哲學（沒有全部找到對應 repo，有些名字在 GitHub 上搜不到確切專案，所以只講思路，不掛連結）。有的不等 context 滿就動手——pai-acp 的思路是讓 agent 自己判斷哪些歷史沒價值了提前壓掉，需要時再搜索、恢復，理由是過多的舊歷史（尤其已經解掉的 bug 討論）只會干擾注意力、誘發幻覺。有的只留一張便利貼——pi-smart-compact 壓縮時只保留當前目標、改過的檔案、錯誤、關鍵決策和未完成事項，因為 agent 真正需要記住的只有「我在解哪個 bug、下一步是甚麼」。有的直接把 context 當 Git 管——pi-context 主動 checkpoint、看 timeline、選擇性 compact，記憶必須可回溯、可 diff、可 revert。有的乾脆不壓縮，改在門口過濾——Hypa 認為最好的壓縮是從一開始就把不相干的垃圾擋在外面，前處理比後處理便宜得多。還有一種連延遲都不讓你感覺到——pi-press 在接近 threshold 時就偷偷在背景先把摘要算好，真正要 compact 時直接切換。
