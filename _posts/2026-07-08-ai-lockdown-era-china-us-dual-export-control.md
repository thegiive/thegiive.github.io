...
**第三，一個地端的 AI harness 框架。** 光有模型和知識庫還不夠，你需要一套框架把它們串起來——模型路由、fallback 切換、知識庫檢索、任務編排。這個不用自己從零造輪子。找 vendor 協助建置，或者用開源方案像 OpenClaw、Hermes 這類 orchestration 框架都可以。重點是：**當某個雲端 API 斷供的時候，這套框架能自動把流量切到地端模型，你的應用不會停擺。** 框架一開始不會做沒關係，但架構要先設計好，讓未來切換的成本盡可能低。

地端知識庫、地端 AI、加上地端 harness 框架，是你在這個不確定時代的保底策略。
