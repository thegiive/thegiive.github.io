...
- [背景](#背景)
- [整體防禦框架](#整體防禦框架)
- [第一層：Isolation（爆炸半徑控制）](#第一層isolation爆炸半徑控制)
- [第二層：Quarantine（內容隔離）](#第二層quarantine內容隔離)
- [第三層：Rollback（回滾能力）](#第三層rollback回滾能力)
- [第四層：Transparency（透明告知）](#第四層transparency透明告知)
...
**Q: 一定要四層都做嗎？可以只做其中幾層嗎？**

四層是相互補強的。Isolation 控制損害範圍，Quarantine 防止惡意輸入，Rollback 確保可復原，Transparency 及早發現問題。少一層，其他層的效果就打折。建議至少先做 Isolation 和 Rollback，這兩層投資報酬率最高。

**Q: 我不是資安專家，這些我做得來嗎？**

這篇文章的每個步驟都是可執行的指令，不需要深厚的資安背景。你甚至可以把這篇文章丟給 Moltbot，讓它幫你執行。重點是「願意花時間認真讀文件」，而不是「要有多少年資安經驗」。

**Q: 密鑰管理用 Bitwarden 還是 1Password？**
