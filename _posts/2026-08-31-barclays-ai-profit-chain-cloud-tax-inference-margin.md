...
faq:
  ...
  - question: "為什麼 AI 推理利潤率能在一年內跳這麼多？"
    answer: "三個力量同時推動。第一，推理效率技術進步——更好的 batching（批次處理）、投機解碼（Speculative Decoding）、模型量化都在降低單次推理的算力成本。第二，MoE（Mixture of Experts，混合專家）稀疏激活架構讓每個 token 只需要運算總參數的一小部分，降低邊際成本。第三，token 需求爆發式成長，固定成本（訓練、基礎設施）被更大的營收基數攤薄。從營收端看，全球 AI 實驗室營收兩年內從 70 億膨脹到 1,370 億美元，訓練支出占營收比從 96% 降到 48%——不是訓練花的錢變少，是推理帶進來的收入漲太快。"
...
利潤率跳升的原因不只一個。推理效率在進步（更好的 batching、speculative decoding、量化），模型架構在省算力（MoE 稀疏激活），同時 token 需求在爆發式成長——量上去之後固定成本被攤薄。這三個力量同時推，利潤率就從十幾跳到六十幾。
...
三個力量同時推動。第一，推理效率技術進步——更好的 batching（批次處理）、投機解碼（Speculative Decoding）、模型量化都在降低單次推理的算力成本。第二，MoE（Mixture of Experts，混合專家）稀疏激活架構讓每個 token 只需要運算總參數的一小部分，降低邊際成本。第三，token 需求爆發式成長，固定成本（訓練、基礎設施）被更大的營收基數攤薄。從營收端看，全球 AI 實驗室營收兩年內從 70 億膨脹到 1,370 億美元，訓練支出占營收比從 96% 降到 48%——不是訓練花的錢變少，是推理帶進來的收入漲太快。
