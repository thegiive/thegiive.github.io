...
Fable 5 到底強在哪？不是「回答更準」——GPQA Diamond 它還輸 Gemini 3.1 Pro。它強在 **調度**：自己拆任務、自己決定哪些子任務丟給便宜模型、自己寫 workflow 腳本讓 runtime 執行。
...
把 Fable 5 放進我們 Harness 系列的脈絡裡看：
...
四個階段的共同方向：**人往上移一個抽象層。** 但每上一層，你需要理解的東西反而更多——因為你需要判斷模型的調度決策對不對。
...
Simon Willison 一天 $110。有開發者一個 code-review 的 prompt「在 subagents 之間燒掉 $92」都沒跑完。
...
6 月 22 日之後免費期結束，需要 usage credits。**「choose agent's model wisely」不只是省錢技巧，是用 Fable 5 的基本功。**

### 安全分類器的誤殺

Fable 5 / Mythos 5 用了一套三域分類器（資安、生化、蒸餾），觸發時會把回應交給 Opus 4.8 處理，API 回傳 `stop_reason: "refusal"`。

問題是分類器太敏感了。有人做醫學影像分析被擋、實驗室自動化被擋、甚至提到 "cancer" 這個字都觸發了生物安全分類器。Anthropic 已經道歉，說他們「做了錯誤的取捨」，95% 的 session 完全沒觸發，但那 5% 的誤殺造成了很大的挫折感。

Nathan Lambert 在 [Interconnects](https://www.interconnects.ai/p/claude-fable-5-and-new-ai-safety) 的批評更尖銳：他認為隱藏的 AI 研究限制是「categorically misaligned AI」——一個在你不知道的情況下自動變笨的 AI。
...
Fable 5 內化的是 **orchestration** 能力——任務分解、模型選擇、並行管理。這是 Harness 的「引擎」。

但 Harness 的另一半——約束、防護、審計——這些是刻意限制模型能做什麼的機制。你不會讓一個模型自己決定「我應不應該被約束」。

如果你的 Harness 還在做 orchestration 的重活——手動分配模型、手動定義 workflow 流程——Fable 5 確實可以接管。但如果你的 Harness 在做的是 [四層防禦](/coders-who-stopped-coding-harness-context-spec-engineering/)、[機械式強制執行](/agent-harness-three-migrations-mechanism/)、[安全最佳實踐](/harness-engineering-security-best-practices/)——這些不但不會被取代，反而因為模型自主性更高，變得更重要了。
...
畢竟，一個會自己調度 Agent 的模型，犯錯的方式也會比一個只會回答問題的模型更多、更快、更大範圍。

**Harness 沒有被吃掉。是引擎內化了，煞車外置了。**
