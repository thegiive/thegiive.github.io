...
| 項目 | 內容 |
|------|------|
| **核心觀點** | 用 AI 做簡報不是為了省時間，而是追求「千人千色」的客製化 |
| **關鍵工具** | NotebookLM + Gemini 2.5 Pro + ChatGPT Atlas + Canva |
| **實際成果** | 100+ 場簡報經驗，對外簡報 NotebookLM 使用率 100% |
| **核心價值** | Customer First — 把客戶放在最中心，追求聽眾滿意度 |
| **適用對象** | 需要對外提案、客戶簡報的專業人士 |
...
| 問題 | 回答 |
|------|------|
| **用 AI 做簡報會不會被看出來？** | 會，如果你直接用預設風格不修改的話。關鍵是要客製化色調、字體、刪除不好的頁面。 |
| **NotebookLM 跟 Gamma.app 差在哪？** | NotebookLM 搭配 Gemini 2.5 Pro 可以更精準理解你的資料，產出的內容「知識保真度」更高。Gamma 比較偏快速美化。 |
| **為什麼要用 ChatGPT Atlas 抓色調？** | Atlas 可以直接操作瀏覽器去看客戶官網，自動分析主色調跟企業價值，比我自己去截圖分析快很多。 |
| **Canva 付費版值得嗎？** | 如果你常做對外簡報，非常值得。字體、素材、背景移除功能都省很多時間。 |
| **內部簡報需要這麼講究嗎？** | 不需要。內部簡報我就是「大綱 → AI 直出 → 直接用」，省力優先。 |
| **這個流程要花多少時間？** | 完整四步驟大約 30-60 分鐘。但產出的品質是以前要 3-4 小時才能達到的。 |
| **沒有設計底子也能做嗎？** | 可以。關鍵是你要會寫好的 Prompt 描述風格，AI 會幫你處理視覺細節。 |
| **這個方法適合什麼場合？** | 對外提案、客戶簡報、重要會議。任何你需要「讓對方感覺被重視」的場合。 |
